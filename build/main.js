...
// .select('(c=>({id,description,Bar:{id,description}}))')
  .where('(c=>(c.id !== p1 || p1 === c.id  && c.id === p0  ))')
// .where('(c => ((c.id1 === p0 || c.Bar.id2 === p1 || c.Bar.Baz.id3 === p2) || c.id4 === p3 && c.id5 === p4))')
...
console.log(context._foo._provider.grammar.where.map(c=> c = typeof c === 'object'? JSON.stringify(c):c).toString());
